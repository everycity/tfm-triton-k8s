...
resource "triton_firewall_rule" "ssh_in" {
  ...
  rule        = "FROM (ip 91.194.74.23 OR ip 95.131.252.106) TO tag \"role\" = \"k8s\" ALLOW tcp PORT 22"
  enabled     = true
}
...
resource "triton_firewall_rule" "k8s-calico-tcp" {
  ...
  rule        = "FROM subnet 10.28.0.0/24 TO tag \"role\" = \"k8s\" ALLOW tcp PORT all"
  enabled     = true
}
...
resource "triton_firewall_rule" "k8s-calico-udp" {
  ...
  rule        = "FROM subnet 10.28.0.0/24 TO tag \"role\" = \"k8s\" ALLOW udp PORT all"
  enabled     = true
}
...
resource "triton_firewall_rule" "http_in" {
  ...
  rule        = "FROM any TO tag \"role\" = \"k8s\" ALLOW tcp PORT 80"
  enabled     = true
}
...
resource "triton_firewall_rule" "https_in" {
  description = "Allow web in"
  rule        = "FROM any TO tag \"role\" = \"k8s\" ALLOW tcp PORT 443"
  enabled     = true
}
